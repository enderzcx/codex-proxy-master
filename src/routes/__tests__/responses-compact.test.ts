...
describe("POST /v1/responses/compact", () => {
  ...
  let app: Hono;
  let historyStore: { record: ReturnType<typeof vi.fn> };
  ...
  beforeEach(() => {
    ...
    historyStore = { record: vi.fn() };
    app = createResponsesRoutes(pool, undefined, undefined, undefined, historyStore as never);
  ...
  it("records request history on success", async () => {
    await app.request("/v1/responses/compact", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    expect(historyStore.record).toHaveBeenCalledTimes(1);
    expect(historyStore.record.mock.calls[0][0]).toMatchObject({
  ...
  it("folds internal retries into one success record", async () => {
    pool.addAccount("test-token-2");
    mockCompactThrow = null;
    mockCompactResponse = { output: [{ role: "assistant", content: "retried" }] };

    let calls = 0;
    vi.mocked(withRetry)
      .mockImplementationOnce(async (_fn: () => Promise<unknown>) => {
        throw new CodexApiError(429, '{"detail":"rate limited"}');
      })
  ...
  it("records request history on auth failure", async () => {
    const emptyPool = new AccountPool();
    const localHistoryStore = { record: vi.fn() };
    const emptyApp = createResponsesRoutes(emptyPool, undefined, undefined, undefined, localHistoryStore as never);
    ...
    expect(res.status).toBe(401);
    expect(localHistoryStore.record).toHaveBeenCalledWith(expect.objectContaining({
      path: "/v1/responses/compact",
      outcome: "error",
