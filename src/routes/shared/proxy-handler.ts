...
export interface ProxyRequest {
  ...
  model: string;
  isStreaming: boolean;
  requestHistory?: RequestHistoryContext;
  ...
  for (;;) {
    try {
        ...
        return stream(c, async (s) => {
          ...
          try {
            ...
            finalizeRequestHistory(requestHistoryStore, accountPool, req.requestHistory, {
              ...
              statusCode: streamResult.aborted ? 499 : c.res.status,
              outcome: streamResult.aborted ? "aborted" : (streamResult.errorMessage ? "error" : "success"),
              errorCode: streamResult.aborted ? "client_aborted" : (streamResult.errorMessage ? "stream_error" : null),
              errorMessage: streamResult.aborted ? "Client disconnected during stream" : streamResult.errorMessage,
              usage: usageInfo,
              attemptCount,
            });
      ...
      return await handleNonStreaming(
        ...
        requestHistoryStore, attemptCount,
      );
    } catch (err) {
      if (!(err instanceof CodexApiError)) {
        finalizeRequestHistory(requestHistoryStore, accountPool, req.requestHistory, {
          ...
          errorMessage: err instanceof Error ? err.message : "Unknown proxy error",
          attemptCount,
        });
    ...
    try {
      ...
      if (collectErr instanceof EmptyResponseError && attempt <= MAX_EMPTY_RETRIES) {
        ...
        try {
          ...
          finalizeRequestHistory(requestHistoryStore, accountPool, req.requestHistory, {
            entryId: currentEntryId,
            statusCode: retryErr instanceof CodexApiError ? toErrorStatus(retryErr.status) : 502,
            outcome: "error",
            errorCode: retryErr instanceof CodexApiError ? "codex_api_error" : "retry_error",
            errorMessage: retryErr instanceof Error ? retryErr.message : "Retry request failed",
            attemptCount,
          });
      ...
      if (collectErr instanceof EmptyResponseError) {
        ...
        finalizeRequestHistory(requestHistoryStore, accountPool, req.requestHistory, {
          ...
          errorMessage: "Codex returned empty responses across all available accounts",
...
export async function handleDirectRequest(
  ...
  try {
    ...
    finalizeRequestHistory(requestHistoryStore, undefined, req.requestHistory, {
      statusCode: status === 429 ? 429 : toErrorStatus(status),
      outcome: "error",
      errorCode: status === 429 ? "rate_limit_error" : "direct_upstream_error",
      errorMessage: msg,
      attemptCount: 1,
    });
    if (status === 429) {
