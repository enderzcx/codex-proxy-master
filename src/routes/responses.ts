...
async function handleCompact(
  ...
  const historyCtx = createRequestHistoryContext(
    c,
  ...
  for (;;) {
    try {
      ...
      finalizeRequestHistory(requestHistoryStore, accountPool, historyCtx, {
        ...
        statusCode: 200,
        outcome: "success",
        attemptCount: triedEntryIds.length,
      });
      ...
      if (!retry) {
        ...
        finalizeRequestHistory(requestHistoryStore, accountPool, historyCtx, {
          ...
          statusCode: status,
          outcome: "error",
          errorCode: decision.useFormat429 ? "rate_limit_exceeded" : "codex_api_error",
          errorMessage: decision.message,
          attemptCount: triedEntryIds.length,
        });
...
export function createResponsesRoutes(
  ...
  const responsesHandler = async (c: Context) => {
    ...
    if (upstreamRouter && !upstreamRouter.isCodexModel(rawModel)) {
      ...
      return handleDirectRequest(c, upstreamRouter.resolve(rawModel), directReq, PASSTHROUGH_FORMAT, requestHistoryStore);
    }

    return handleProxyRequest(c, accountPool, cookieJar, proxyReq, PASSTHROUGH_FORMAT, proxyPool, requestHistoryStore);
  };
