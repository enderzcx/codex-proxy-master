...
import { RequestHistoryStore } from "./auth/request-history.js";
...
export async function startServer(options?: StartOptions): Promise<ServerHandle> {
  ...
  const chatRoutes = createChatRoutes(accountPool, cookieJar, proxyPool, upstreamRouter, requestHistory);
  const messagesRoutes = createMessagesRoutes(accountPool, cookieJar, proxyPool, upstreamRouter, requestHistory);
  const geminiRoutes = createGeminiRoutes(accountPool, cookieJar, proxyPool, upstreamRouter, requestHistory);
  const responsesRoutes = createResponsesRoutes(accountPool, cookieJar, proxyPool, upstreamRouter, requestHistory);
  ...
  const webRoutes = createWebRoutes(accountPool, usageStats, requestHistory);
  ...
  app.route("/", createEmbeddingsRoutes(accountPool, requestHistory));
